...
| **read-before-edit** | `session_start`, `tool_call`, `tool_result` | Tracks which files (and their latest `[PATH#TAG]`) were read this session; blocks an `ast_edit`/write-family call to an unread file (`write` is exempt — it can create new files). Also reconciles a truncated hashline edit header path (`[binding.go#665A]` → `[pkg/aether/binding.go#665A]`) when the basename **and** tag uniquely match a read file — the tag match makes a wrong rewrite practically impossible. | on |
| **redundant-read-guard** | `session_start`, `tool_result` | Suppresses the body of a `read` that re-delivers content already shown this session (same `[PATH#TAG]`, line span ⊆ what was shown), replacing it with a compact pointer to the snapshot the model already holds. Edits reset a path's shown-set to the changed region. Cuts the re-read-to-refresh-tag amplification that bloats history under `-np 1` + prompt cache; safe because it compares the actual returned tag, so any real change flows through. Only collapsed structural-summary rows are *not* counted as "shown" (so a re-read that expands a body still flows), and only large re-injects (≥30 lines) are suppressed — small re-reads pass through for recency. | on |
